...
local function addHoverGrow(btn, scaleUp)
	...
	btn.MouseLeave:Connect(function() tw(us, 0.12, {Scale = 1}) end)
	btn.MouseButton1Down:Connect(function() tw(us, 0.08, {Scale = 0.97}) end)
	btn.MouseButton1Up:Connect(function() tw(us, 0.12, {Scale = 1.04}) end)
	return us
end
local staggerIdx = 0
...
local logoFloatConn = RunService.Heartbeat:Connect(function()
	...
	glowWrap.Position = UDim2.new(0, 0, 0, math.sin(tick() * 1.2) * 4)
end)
task.spawn(function()
	while gui.Parent do
...
mkLbl(headerCG, "Key System", 22, C_WHITE, Enum.Font.GothamBold, Enum.TextXAlignment.Center, 3, 32)
mkLbl(headerCG, "A key is required to use the Autofarm Script.", 13, C_SUB, Enum.Font.Gotham, Enum.TextXAlignment.Center, 4, 20)
spacer(headerCG, 5, 6)
...
mkLbl(discTextBlock, "Join our Discord!", 14, C_WHITE, Enum.Font.GothamBold, Enum.TextXAlignment.Left, 1, 18)
...
joinBtn.MouseButton1Click:Connect(function()
	...
	joinBtn.Text = "  ✓  Copied!"
	task.delay(1.8, function() if joinBtn and joinBtn.Parent then joinBtn.Text = orig end end)
end)
spacer(modal, 5, 14)
...
mkLbl(gkCG, "Choose how you would like to obtain your key.", 12, C_SUB, Enum.Font.Gotham, Enum.TextXAlignment.Left, 2, 16)
...
local _crPad = Instance.new("UIPadding", cardsRow)
_crPad.PaddingTop = UDim.new(0,3) _crPad.PaddingBottom = UDim.new(0,3)
_crPad.PaddingLeft = UDim.new(0,2) _crPad.PaddingRight = UDim.new(0,2)
...
local function makeOptionCard(order, iconId, title, desc, btnTxt, btnColor, btnTextColor, hoverColor, btnCb, iconBgColor, cardHoverColor)
	...
	iImg.LayoutOrder = 1
	spacer(card, 2, 7)
	mkLbl(card, title, 13, C_WHITE, Enum.Font.GothamBold, Enum.TextXAlignment.Center, 3, 15)
	spacer(card, 4, 4)
	...
	btn.MouseButton1Click:Connect(function()
		btnCb()
		local origTxt = btn.Text local origCol = btn.TextColor3
		btn.Text = "  ✓  Copied!"
		tw(btn, 0.1, {TextColor3 = C_GREEN})
		task.delay(1.8, function()
			if btn and btn.Parent then btn.Text = origTxt tw(btn, 0.1, {TextColor3 = origCol}) end
		end)
	end)
	card.MouseEnter:Connect(function() tw(card, 0.15, {BackgroundColor3 = cardHoverColor or Color3.fromRGB(21, 21, 27)}) end)
	card.MouseLeave:Connect(function() tw(card, 0.15, {BackgroundColor3 = C_CARD}) end)
end
 
-- Get Key button — uses Junkie get_key_link via Arqel internally
-- We open the checkpoint link directly
local Arqel = nil
task.spawn(function()
	pcall(function()
		Arqel = loadstring(game:HttpGet("https://cdn.jnkie.com/arquelui.lua"))()
		if Arqel then
			Arqel.Appearance.Title    = "ATM Farmer"
			Arqel.Appearance.Icon     = "rbxassetid://134697043118282"
			Arqel.Links.Discord       = DISCORD_URL
			Arqel.Storage.FileName    = KEY_FILE
			Arqel.Theme.Accent        = Color3.fromRGB(34,  197, 94)
			Arqel.Theme.Background    = C_BG
			Arqel.Theme.Header        = C_CARD
			Arqel.Theme.Input         = C_INPUT
			Arqel.Theme.Text          = C_WHITE
			Arqel.Theme.TextDim       = C_SUB
			Arqel.Theme.Success       = C_GREEN
			Arqel.Theme.Error         = Color3.fromRGB(220, 55, 55)
		end
	end)
end)
...
makeOptionCard(1, A_KEY, "Get Key", "Free 1-hour trial available.\nComplete an offer to get access.",
	...
	function()
		task.spawn(function()
			-- Use Junkie SDK to get the checkpoint link
			local ok, lib = pcall(function()
				local j = loadstring(game:HttpGet("https://jnkie.com/sdk/library.lua"))()
				j.service    = JUNKIE_SERVICE
				j.identifier = JUNKIE_IDENTIFIER
				j.provider   = JUNKIE_PROVIDER
				return j
			end)
			if ok and lib then
				local link, err = lib.get_key_link()
				if link then
					openURL(link)
				else
					openURL(DISCORD_URL)
...
makeOptionCard(3, A_DISCORD, "Join Discord", "Join for support, updates\n& announcements.",
	...
	Color3.fromRGB(20, 22, 55), Color3.fromRGB(16, 17, 38))
...
mkLbl(ekCG, "Paste your key below and click Verify.", 12, C_SUB, Enum.Font.Gotham, Enum.TextXAlignment.Left, 2, 16)
...
local _irPad = Instance.new("UIPadding", inputRow)
_irPad.PaddingTop = UDim.new(0,2) _irPad.PaddingBottom = UDim.new(0,2)
_irPad.PaddingLeft = UDim.new(0,2) _irPad.PaddingRight = UDim.new(0,2)
...
local inputIco = mkImg(inputFrame, A_KEY, UDim2.new(0, 16, 0, 16), UDim2.new(0, 14, 0.5, -8), C_SUB)
...
verifyBtn.MouseEnter:Connect(function()
	if verifyBtn.Text ~= "..." then tw(verifyBtn, 0.15, {BackgroundColor3 = C_GHOVER}) end
end)
verifyBtn.MouseLeave:Connect(function()
	if verifyBtn.Text ~= "..." then tw(verifyBtn, 0.15, {BackgroundColor3 = C_GREEN}) end
end)
...
local function startDotPulse()
	...
	dotPulseConn = {Disconnect = function() active = false end}
end
local function setStatus(state, msg)
	...
	if state == "wait" then
		tw(statusDot, 0.2, {BackgroundColor3 = C_SUB}) statusTxt.TextColor3 = C_SUB startDotPulse()
	elseif state == "check" then
		tw(statusDot, 0.2, {BackgroundColor3 = C_YELLOW}) statusTxt.TextColor3 = C_YELLOW startDotPulse()
	elseif state == "ok" then
		tw(statusDot, 0.2, {BackgroundColor3 = C_GREEN}) statusTxt.TextColor3 = C_GREEN
	elseif state == "err" then
		tw(statusDot, 0.2, {BackgroundColor3 = C_RED}) statusTxt.TextColor3 = C_RED
	end
end
startDotPulse()
...
-- ══════════════════════════════════════════════════════════════
-- VERIFY LOGIC — uses Arqel:LaunchJunkie() invisibly
-- ══════════════════════════════════════════════════════════════
...
local function doVerify()
	...
	if key == "" then
		...
		task.spawn(shakeInput) task.spawn(flashRed)
		return
	end
	...
	task.spawn(function()
		...
		-- ── KEY POINT: set SCRIPT_KEY then launch Arqel invisibly ──
		-- We never call check_key() — Arqel:LaunchJunkie() handles validation
		-- and loading the Original Code in one shot with no double-check
		...
		-- Set tier as free trial
		getgenv().IS_PREMIUM       = false
		getgenv().USER_TIER        = "trial"
		getgenv().TIER_LABEL       = "Free Trial"
		getgenv().LICENSE_TIMELEFT = 3600
		getgenv().LICENSE_STATUS   = "Active"
 
		-- Now let Arqel handle the rest — it validates the key and loads Original Code
